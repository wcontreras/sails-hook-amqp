...
Amqp.prototype.subscribe = function subscribe(q, onMessage, assertQueueOpts, consumeOpts) {
    ...
    this.connection.createChannel(function (err, ch) {
        ...
        function processMsg(msg) {
            try {
                if (msg == null || msg == '' || typeof onMessage !== 'function') {
                    return ch.ack(msg);
                }

                try {
                    return onMessage(ch, JSON.parse(msg.content.toString()), msg)
                } catch (e) {
                    return onMessage(ch, msg.content.toString(), msg)
                }
